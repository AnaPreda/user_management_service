defmodule UserManagementService.Auth do
  use GenServer
  use Timex
  ...
  alias UserManagementService.Helpers.MapHelper
...
#  @issuer Path.join([
#    Application.get_env(:users, :api_host),
#    Application.get_env(:users, :api_version),
#    Application.get_env(:users, :api_prefix)
#  ])
  ...
  def handle_call({:issue_token, claims}, _from, state) when is_map(claims) do
    jwt = claims |> Map.merge(%{
      ...
      exp: Timex.to_unix(Timex.shift(Timex.now, days: @jwt_validity))
          ...
          |> token
          |> with_signer(hs256(:base64.encode(@app_secret_key)))
          |> sign
          |> get_compact
